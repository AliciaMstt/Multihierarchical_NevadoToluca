...
for (i in 1:length(unique (s2_raw_threshold$limite0.03)))
{
  ...
  subcom_level_names0.03[,c(2:51)]->subcom_level  #delete names, level and also the negative column
  colSums(subcom_level)->sum0.03
  as.data.frame(sum0.03)->sum0.03
  t(sum0.03)->sum0.03
  row.names(sum0.03)<-subcom_level_names0.03[1,1] #keep the name of the first haplotype
  rbind(s2_raw_Hymenoptera_limite0.03,sum0.03)->s2_raw_Hymenoptera_limite0.03
}
...
s2_raw_Hymenoptera_limite0.03[,1:50]->data0.03
rowSums(data0.03)
...
t_s2_f4_Hymenoptera_limite0.03[1:50,]->community_Hymenoptera_limite0.03 #NOTA_Nancy: Este numero es importante. Colocar exactamente el numero de "s2_f4[,2:52]->data0.03".
...
dim(community_Hymenoptera0.03)
community_Hymenoptera0.03[which(str_extract (row.names(community_Hymenoptera0.03), "_NTO_") %in% "_NTO_"),]->community_Hymenoptera_Site0.03
dim(community_Hymenoptera_Site0.03)
community_Hymenoptera_Site0.03[,which(colSums(community_Hymenoptera_Site0.03)!=0)]->community_Hymenoptera_Site0.03 ##to remove no data colums
dim(community_Hymenoptera_Site0.03)
write.table (community_Hymenoptera_Site0.03, file="../genetic/Data_out/Hymenoptera/Hymenoptera3P/community_Hymenoptera_Site0.03.txt") ##this is necessary for the format, not able to solve in other way
...
##**BY SITE**
row.names(community_Hymenoptera_Site0.03)->sample_names_Mountain1
as.data.frame(sample_names_Mountain1)->sample_names_Mountain1
sample_names_Mountain1 %>% separate(sample_names_Mountain1, c("Conservation","Mountain1","Site","ID"), sep="_",remove=FALSE)->general_sample_Mountain1Site0.03
...
##**Hymenoptera**
as.matrix(community_Hymenoptera_Site0.03)->community_Hymenoptera_Site0.03
row.names(community_Hymenoptera_Site0.03)->sample_names_Site0.03
dim(community_Hymenoptera_Site0.03)->dims_Site0.03
dims_Site0.03
.rowSums (community_Hymenoptera_Site0.03,dims_Site0.03[1],dims_Site0.03[2])->sample_richness_Site0.03 ##summatory by rows
...
##Singletons by SITE
##summatory by rows
colnames(community_Hymenoptera_Site0.03)->h_names_Site0.03
dim (community_Hymenoptera_Site0.03)->dims_Site0.03
.colSums (community_Hymenoptera_Site0.03,dims_Site0.03[1],dims_Site0.03[2])->h_ocurrence_Site0.03 ##summatory by cols
...
##**community_Hymenoptera_Site0.03**
##singletons
community_Hymenoptera_Site0.03[,singletons_Site0.03]->community_Hymenoptera_singletons_Site0.03
row.names(community_Hymenoptera_singletons_Site0.03)->sample_names_Site0.03
dim (community_Hymenoptera_singletons_Site0.03)->dims_Site0.03
dims_Site0.03
.rowSums (community_Hymenoptera_singletons_Site0.03,dims_Site0.03[1],dims_Site0.03[2])->sample_richness_singletons_Site0.03 ##summatory by rows
...
plot(cum_Site0.03, cex=1.4, cex.lab=1.4, cex.axis=2.3, lwd=3, ylim=c(0,100), main="h_Hymenoptera_Site_0.03")
specpool(community_Hymenoptera_Site0.03)->specpool_Site0.03
specpool_Site0.03$Species/specpool_Site0.03$chao*100
text(x=38, y=5, labels="66.69%", cex=1.5)
#
...
plot (MDSbetasim0.03, main="Hymenoptera_Site0.03")
x<- MDSbetasim0.03$points[,1]
y<- MDSbetasim0.03$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Hymenoptera_sinoutlayer0.03))

plot (MDSbetasim0.03, main="Hymenoptera_Site_0.03")
with(general_sample_sinoutlayer0.03,ordispider(MDSbetasim0.03, Site, label=T, col="blue"))

plot (MDSbetasim0.03, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.42), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Hymenoptera_Site_0x.03")
with(general_sample_sinoutlayer0.03,ordispider(MDSbetasim0.03, Site, label=T, cex.lab=0.9, col= c("#153a7b", "#eaa22f", "#97518b", "#81b9d0")))

plot (MDSbetasim0.03, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.42), cex=2, cex.lab=1, cex.axis=2.3, lwd=4.5, main="Hymenoptera_Site_0.03")
