...
Collembola_UPGMA <- upgma(dist_Collembola)  #hacer árbol
...
Collembola_UPGMA.GMYC.singleThreshold<-gmyc.edit(Collembola_UPGMA, method="s") #hacer GMYC
#resumen de los resultados
summary(Collembola_UPGMA.GMYC.singleThreshold) #Mostrar resumen GMYC

sink("Collembola/SpeciesDelimitation/Collembola_UPGMA.GMYC.singleThreshold.log", type=c("output","message"), split=TRUE) #guardar resultado del GMYC (resumen)
#MIT.test.s o MIT.test.m para ver todos los datos
summary(Collembola_UPGMA.GMYC.singleThreshold)
sink()
