...
#**Community diversity and composition at the haplotype level of the Hemiptera order**
...
s2_raw_all_h <- read.table("../genetic/Data_in/Hemiptera/s2_raw_all_Hemiptera_threshold.txt", header=TRUE)
dim(s2_raw_all_h)

#**Table Haplotipos**
#####remove additional columns and leave only names (of haplotipes), samples and taxa (and threshold in this case)
s2_raw_all_h[,c(1:52,68)]->s2_raw_h 
...
s2_raw_h[which(s2_raw_h$conservative_threshold == "1"),]->s2_raw_threshold_h 
s2_raw_threshold_h [,1:52]->s2_raw_threshold_h ##remove threshold col
dim(s2_raw_threshold_h)
colnames(s2_raw_threshold_h)

##**transform in present/absence table**
s2_raw_threshold_h->s2_f4_h #NOTA_Nancy: Tengo un subset de Hemiptera
...
write.table (community_Hemiptera_h, file="../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/community_Hemiptera_h.txt") ##this is necessary for the format, not able to solve in other way
...
dim(community_Hemiptera_h)
community_Hemiptera_h[which(str_extract (row.names(community_Hemiptera_h), "_NTO_") %in% "_NTO_"),]->community_Hemiptera_Site_h
...
##**BY SITE**
##**Generating a general table with names and habitat parameters**
row.names(community_Hemiptera_Site_h)->sample_names_Mountain1_h
...
general_sample_Mountain1Site_h
write.table(general_sample_Mountain1Site_h, file="../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/general_sample_Mountain1Site_h.txt") ##this is the only way I found to be able to work later
...
##**Hemiptera**
as.matrix(community_Hemiptera_Site_h)->community_Hemiptera_Site_h
row.names(community_Hemiptera_Site_h)->sample_names_Site_h
dim(community_Hemiptera_Site_h)->dims_Site_h
dims_Site_h
.rowSums (community_Hemiptera_Site_h,dims_Site_h[1],dims_Site_h[2])->sample_richness_Site_h ##summatory by rows
...
##**BY SITE**
write.table(richness_Site_h, file="../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/richness_Site_h_Hemiptera.txt") ##this is the only way I found to be able to work later
read.table("../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/richness_Site_h_Hemiptera.txt",header=TRUE)->richness_Site_h

##**BY SITE_C general**
write.table(richness_SiteC, file="../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/richness_SiteC_Hemiptera.txt") ##this is the only way I found to be able to work later
read.table("../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/richness_SiteC_Hemiptera.txt",header=TRUE)->richness_SiteC

##**General plot of richness by sample in SITE**
barplot(richness_Site_h$sample_richness_Site_h,col=richness_Site_h$Mountain1Site,names.arg= richness_Site_h$sample_names_Site_h, las=2,cex.names=0.5, ylab="richness_Site_h", main="H richness_Site Hemiptera")
...
##Singletons by SITE
##summatory by rows
colnames(community_Hemiptera_Site_h)->h_names_Site
dim (community_Hemiptera_Site_h)->dims_Site_h
.colSums (community_Hemiptera_Site_h,dims_Site_h[1],dims_Site_h[2])->h_ocurrence_Site ##summatory by cols
...
write.table(h_ocurrence_Site, file="../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/h_ocurrence_Site_Hemiptera.txt") ##this is the only way I found to be able to work later
read.table("../genetic/Data_out/Hemiptera/Hemiptera_Haplotypes/h_ocurrence_Site_Hemiptera.txt",header=TRUE)->h_ocurrence_Site
...
##community_Hemiptera_Site_h
##singletons
community_Hemiptera_Site_h[,singletons_Site_h]->community_Hemiptera_singletons_Site_h
row.names(community_Hemiptera_singletons_Site_h)->sample_names_Site_h
dim (community_Hemiptera_singletons_Site_h)->dims_Site_h
dims_Site_h
.rowSums (community_Hemiptera_singletons_Site_h,dims_Site_h[1],dims_Site_h[2])->sample_richness_singletons_Site_h ##summatory by rows
...
plot(cum_Site_h, cex=1.4, cex.lab=1.4, cex.axis=2.3, lwd=3, main="h_Hemiptera_Site")
specpool(community_Hemiptera_Site_h)->specpool_Site_h
specpool_Site_h$Species/specpool_Site_h$chao*100
text(x=38, y=5, labels="58.71%", cex=1.5)
#
...
plot (MDSbetasim_h, main="Hemiptera_Site_h")
with(general_sample_sinoutlayer_h,ordispider(MDSbetasim_h, Site, label=T, col="blue"))
##plot (MDSbetasim_h, main="Hemiptera_Site")
##with(general_sample_sinoutlayer_h,ordispider(MDSbetasim_h, ID, label=T, col="red"))
plot (MDSbetasim_h, main="Hemiptera_Siteh")
x<- MDSbetasim_h$points[,1]
y<- MDSbetasim_h$points[,2]
text(x, y, pos = 1, cex=0.7, labels = row.names (community_Hemiptera_sinoutlayer_h))

plot (MDSbetasim_h, main="Hemiptera_Site_h")
with(general_sample_sinoutlayer_h,ordispider(MDSbetasim_h, Site, label=T, col="blue"))

plot (MDSbetasim_h, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.49), cex.axis=1.4, cex=1.2, cex.lab=1.4, main="Hemiptera_Site_h")
with(general_sample_sinoutlayer_h,ordispider(MDSbetasim_h, Site, label=T, cex.lab=0.9, col= c("#153a7b", "#eaa22f", "#97518b", "#81b9d0")))

plot (MDSbetasim_h, xlim=c(-0.5, 0.5), ylim=c(-0.4, 0.49), cex=2, cex.lab=1, cex.axis=2.3, lwd=4.5, main="Hemiptera_Site_h")
